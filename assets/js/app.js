/*!
  * App js v1.1.0
	* Library to use devices that compatible with 4DS Lab products.
	* Includes socket io v2.3.0 (c) 2014-2019 Guillermo Rauch with Released under the MIT License.
	* Includes JS Cookies v2.2.1 (c) 2006, 2015 Klaus Hartl & Fagner Brack with Released under the MIT License.
	* Includes Regression.JS - Regression functions for javascript v2.0.1 (c) 2013 Tom Alexander with Released under the MIT License.
  * Copyright 2020 4DS Team
...
// function to list devices port that can be used
function deviceLists(callback) {
	return new Promise((resolve,reject) => {
		fetch("http://localhost:8888/device/lists")
		.then(async response => {
		...
		.catch(async error => {
			if (typeof callback == "function") {
				return callback(await error,null);
			} else {
				reject(error);
			}
		})
...
// function to list baud rate that can be used
function baudRateLists(callback) {
	return new Promise((resolve,reject) => {
		fetch("http://localhost:8888/device/baudrate")
		.then(async response => {
			if (typeof callback == "function") {
				return callback(null,await response.json());
			} else {
...
// function to use device port
function useDevice(port,baud,callback) {
	return new Promise((resolve,reject) => {
		fetch("http://localhost:8888/device/use", {
			method: "POST",
			headers: {
		...
		.then(async response => {
			var serialPort = await response.json();
			if (typeof callback == "function") {
				// if (serialPort.msg.includes("is opened")) {
					// socket.emit("ready", { signal: 1 })
				// }
...
// define a new event to receive data with socket io.
...
// define a new event to receive news with socket io.
socket.on("news", function (data) {
	var evt = new CustomEvent("receiveNews", {
		detail: {
			data: data.data
		},
		bubbles: true,
